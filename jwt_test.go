...
func newJwt() *JWT {
	...
	rsaAlgorithms := map[string]crypto.Hash{
		"RS256": crypto.SHA256,
		"RS384": crypto.SHA384,
		"RS512": crypto.SHA512,
	}
	...
	// Add RSAAlgorithms.
	for name, hash := range rsaAlgorithms {
	...
	hmacAlgorithms := map[string]crypto.Hash{
		"HS256": crypto.SHA256,
		"HS384": crypto.SHA384,
		"HS512": crypto.SHA512,
	}
	// Add HMACAlgorithms.
	for name, hash := range hmacAlgorithms {
		a, err := NewHMACAlgorithm(hash, hmacKey)
		if err != nil {
			panic(err)
		}
		jwt.AddAlgorithm(name, a)
